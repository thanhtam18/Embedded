...
void I2C_LCD_WriteVal(uint8_t u8Cmd)
{
...
void sendString_XY(uint8_t row, uint8_t column, char *str){
	switch(row){
		case 1:
		I2C_LCD_WriteCmd(0x80 | ( 0x0F & column));
		break;
		case 2:
		I2C_LCD_WriteCmd(0xC0 | ( 0x0F & column));
		break;
	}
	I2C_LCD_Puts(str);
}

void sendVal_XY(uint8_t row, uint8_t column, uint8_t val){
	switch(row){
		case 1:
		I2C_LCD_WriteCmd(0x80 | ( 0x0F & column));
		break;
